...
class VLLMCollector:
    ...
    def collect(self, num_episodes: int) -> Dict:
        ...
        def run_episode(episode_id: int) -> None:
            ...
            while not done:
                ...
                step_data = {
                    ...
                    "formatted_observation": result["formatted_observation"],
                    "reasoning": reasoning,
                    "action": action,
                    "step": step_count,
            ...
            # Incrementally write this episode to details.csv
            self._write_episode_to_details_csv(self.iteration, episode_id, episode_data)
            
            # We don't need to return episode data anymore since we're writing it to CSV
            # Just return metrics for progress tracking
            return {
                "episode_id": episode_id,
                "tokens": token_tracker.episode_tokens,
                "time": token_tracker.episode_time
            }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                # We don't need the episode_data anymore
                future.result()
        ...
        # Prepare return dictionary - no need to return all collected data
        metrics = {
            "token_metrics": {
    ...
    def _write_eval_episode_to_detailed_eval_csv(self, iteration: int, eval_info: Dict[str, Any]):
        """Write a single evaluation episode to the detailed evaluation CSV."""
        if not eval_info:
            return 
        ...
        with open(self.eval_details_csv_path, "a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                iteration,
                eval_info.get("set"),
                eval_info.get("env_id"),
                eval_info.get("episode_id"),
                self.checkpoint_paths[0] if self.checkpoint_paths else "unknown",
                eval_info.get("model_player_id"),
                eval_info.get("model_reward"),
                eval_info.get("opponent"),
                eval_info.get("opponent_reward"),
    ...
    def _write_eval_summary_to_eval_csv(self, iteration: int, eval_summary_dict: Dict[str, Dict[str, Any]]):
        """Write a summary of an evaluation run to the evaluation summary CSV."""
        if not eval_summary_dict:
            return 
