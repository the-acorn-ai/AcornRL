...
#     """
#     An agent that uses a language model to generate actions and can be trained with REINFORCE.
#     """
...
#         Args:
#             observation: The environment observation (string)
...
#         action = self.model.generate_text(
#             prompt=observation,
...
#         """
#         # Convert rewards to tensor and reshape
#         # rewards_tensor = torch.tensor(rewards, device=self.model.device)
#         self.model.train()
        
#         # Compute log probabilities for each action given its observation
#         log_probs = []
#         rewards = []
#         for sample in batch:
#             # TODO add reasoning
#             action_log_probs = self.model.compute_logprobs(sample["observation"], sample["action"])
#             # Sum log probs over tokens to get sequence log prob
#             log_probs.append(action_log_probs.sum())
#             rewards.append(sample["final_reward"]*self.gamma**(sample["full_length"]-sample["step"]))
#         # Stack all log probs
#         log_probs_tensor = torch.stack(log_probs).to("cuda")
#         print(rewards)
#         rewards_tensor = torch.tensor(rewards).to("cuda")
#         # Compute the REINFORCE loss: -log(p(a|s)) * reward
#         # Negative because we want to maximize reward (gradient ascent)
#         # but optimizer does gradient descent
...
#     ) -> torch.Tensor:
#         """
#         Calculate loss for a single observation-action-reward triplet.
