...
class StandardTrainer:
    """
    Standard trainer for reinforcement learning with language models.
    Handles structured outputs with reasoning and answer separation.
    """
    ...
    def _run_training_step(self, episodes_per_epoch):
        ...
        try:
            # Collect structured data with reasoning and action separated
            data = self.collector.collect(agent1=self.agent, agent2=self.agent, num_episodes=episodes_per_epoch)
            
            if not data:
                self.logger.warning("No data collected in this epoch")
                return 0.0
            ...
            try:
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    self.logger.warning(f"Invalid loss value: {loss}. Skipping update.")
                    return 0.0
                ...
                torch.nn.utils.clip_grad_norm_(self.agent.model.parameters(), max_norm=self.grad_clip)
                ...
                self.logger.error(f"Error in loss computation or optimization: {e}")
                return 0.0
    ...
    def _run_eval_step(self):
        ...
        try:
            # Simple evaluation: run a few episodes and compute average reward
            eval_episodes = max(1, min(2, self.collector.batch_size // 4))
            eval_data = self.collector.collect(agent1=self.agent, agent2=self.agent, num_episodes=eval_episodes)
            ...
            for sample in eval_data:
                total_reward += sample.get("final_reward", 0.0)
                count += 1
